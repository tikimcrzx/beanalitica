...
let BranchesService = class BranchesService {
    ...
    async menu(name) {
        ...
        for (let index = 0; index < branch.menu.length; index++) {
            array.push(this.messageBots(branch.menu[index].name, branch.menu[index].name, branch.menu[index].name, branch.menu[index].image));
        }
        return array;
    }
    ...
    async responseBot(name, intent) {
        ...
        if (intent === intents_enum_1.Intents.LOCATION_RESTAURANT) {
            object = this.messageBot(`Hola estamos ubicados en ${branch.address}`, 'restaurantLocation');
        }
        else if (intent === intents_enum_1.Intents.SCHEDULE_RESTAURANT) {
            object = this.messageBot(`Hola nuestro horario es ${branch.schedule}`, 'restaurantSchedule');
        }
        return object;
    }
};
